...
import { asEvidenceArray, normalizeScoreReasoning } from '@/lib/scoring-normalize'
...
export function getDisplayEvidence(candidate: CandidateLike): EvidenceItem[] {
  const reasoning = { ...FALLBACK_REASONING, ...normalizeScoreReasoning(candidate.scoring?.score_reasoning) }
  const existing = asEvidenceArray(candidate.scoring?.evidence)
