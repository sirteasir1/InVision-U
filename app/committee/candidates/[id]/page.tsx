...
export default function CandidateDetailPage() {
  ...
  const reasoning = useMemo(() => ({ ...FALLBACK_REASONING, ...(scoring?.score_reasoning ?? {}) }), [scoring])
  const authorship = useMemo(
    () => ({ ...FALLBACK_AUTHORSHIP, ...(scoring?.authorship_assessment ?? {}), signals: scoring?.authorship_assessment?.signals ?? [] }),
    [scoring]
  )
