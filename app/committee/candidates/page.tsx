    ...
    <div className="px-4 md:px-10 pb-16 max-w-[1440px] mx-auto">
      <div className="py-10 space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <button onClick={load} className="flex items-center gap-2 text-on-surface-variant hover:text-primary-container text-sm font-label font-bold transition-colors self-start md:self-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={stat.label} className="bg-surface-container-low rounded-xl px-6 py-5">
      ...
      <div className="mb-5 relative">
        ...
        <input className="iv-input pl-12" placeholder="Поиск по имени или email…" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>

      {loading ? (
        <div className="bg-surface-container-low rounded-xl p-16 text-center space-y-3">
        ...
        <div className="bg-surface-container-low rounded-xl overflow-hidden">
          ...
          <div>
            {filtered.map((c, i) => (
              <div key={c.id} className={`grid grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-surface-container transition-colors duration-150 ${i !== 0 ? 'border-t border-surface-container-high/40' : ''}`}>
                ...
                <div className="col-span-6 md:col-span-2 flex items-center justify-end gap-2">
                  <Link href={`/committee/candidates/${c.id}`} className="flex items-center gap-2 text-primary-container hover:text-white text-sm font-label font-bold transition-colors">
                    <span className="material-symbols-outlined text-base">open_in_new</span>
                    Открыть
                  </Link>
                  <button onClick={() => del(c.id, c.full_name)} disabled={deleting === c.id} className="flex items-center gap-2 text-error hover:text-error-dim text-sm font-label font-bold transition-colors disabled:opacity-50">
